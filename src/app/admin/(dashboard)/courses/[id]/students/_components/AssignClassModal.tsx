...
export function AssignClassModal({
    ...
    const handleAssign = async () => {
        ...
        try {
            // Updated endpoint to /assign-students
            const res = await api.post(`/admin/classes/${selectedClassId}/assign-students`, {
                student_ids: studentIds
            })

            if (res.data?.success) {
                toast.success(`Đã thêm ${studentIds.length} học sinh vào lớp ${classes.find(c => c.id === selectedClassId)?.class_code}`)
                onOpenChange(false)
                onSuccess?.()
            } else {
