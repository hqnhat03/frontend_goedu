...
export default function ClassDetailPage() {
  ...
  React.useEffect(() => {
    const fetchClassDetail = async () => {
      try {
        setLoading(true)
        const response = await api.get(`/teacher/classes/${params.id}`)
                ...
                <Table>
                  <TableHeader className="bg-muted/30">
                    <TableRow className="hover:bg-transparent border-border/20">
                      ...
                      <TableHead className="font-bold text-xs uppercase tracking-wider">Mã số</TableHead>
                      <TableHead className="font-bold text-xs uppercase tracking-wider">Thông tin liên hệ</TableHead>
                    ...
                    {filteredStudents.length === 0 ? (
                      ...
                      filteredStudents.map((student, index) => (
                        <TableRow key={student.id} className="hover:bg-muted/30 transition-colors border-border/10">
                          ...
                          <TableCell>
                            <Badge variant="outline" className="font-mono text-[10px] py-0 px-2 rounded-md bg-muted/30">
                              #{student.id}
                            </Badge>
                          </TableCell>
                          <TableCell>
