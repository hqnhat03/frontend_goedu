...
export default function ExamQuestionsPage() {
  ...
  const fetchQuestions = React.useCallback(async () => {
    setIsLoading(true)
    try {
      const response = await api.get(`/teacher/exams/${examId}/questions`)
      const data = response.data.data || []
      data.sort((a: Question, b: Question) => (a.order_number || 0) - (b.order_number || 0))
      setQuestions(data)
    } catch (error) {
    ...
    <div className="container max-w-5xl mx-auto py-8 px-4 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 z-20 bg-background/80 backdrop-blur-md py-4 border-b">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight text-primary">Cập nhật câu hỏi</h1>
      ...
      <div className="space-y-8 pb-24">
        {questions.length === 0 ? (
          ...
          questions.map((q, index) => {
            const isEditing = editingIds.includes(q.id)
              ...
              return (
                ...
                >
                  <CardContent className="p-4 flex gap-4 items-start relative">
                    <div className="flex flex-col items-center gap-1 mt-1 cursor-grab active:cursor-grabbing text-muted-foreground/30 hover:text-primary transition-colors">
              ...
              >
                ...
                <CardHeader className="bg-muted/30 border-b flex flex-row items-center justify-between p-3 px-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        <SelectContent className="rounded-lg border-none shadow-2xl">
                          ...
                          <SelectItem value="essay" className="font-bold">
                            Tự luận
                          </SelectItem>
                        </SelectContent>
                      </Select>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1 bg-background px-2 py-1 rounded-lg border shadow-sm">
                      ...
                      <input
                        ...
                        value={q.score}
                        onChange={(e) =>
                          updateQuestion(q.id, { score: parseFloat(e.target.value) || 0 })
                        }
                        className="w-8 bg-transparent border-none text-sm font-black focus:ring-0 text-center text-primary p-0"
                      />
                    </div>
                    <Button
                      variant="ghost"
