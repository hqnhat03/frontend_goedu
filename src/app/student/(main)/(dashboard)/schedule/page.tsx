                    ...
                    <div className="flex flex-col select-none relative">
                        {TIMES.map((time, rowIdx) => (
                            <div key={time.start} className="grid grid-cols-[64px_repeat(7,minmax(0,1fr))] hover:bg-slate-50/30 transition-colors">
                                <div className="p-1.5 text-[10px] font-bold text-center text-slate-400 flex items-center justify-center border-r border-slate-200 bg-slate-50/20 h-[40px] border-b border-slate-100/50">
                                    {time.label}
                                </div>
                                {weekDays.map((day) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {session ? (
                                                <div
                                                    className={cn(
                                                        "w-full h-full min-h-[32px] transition-all duration-300 flex flex-col items-center justify-center overflow-hidden relative",
                                                        "bg-blue-600 text-white shadow-sm",
                                                        !isPrevSelected && "rounded-t-xl",
                                                        !isNextSelected && "rounded-b-xl shadow-md",
                                                ...
                                                >
                                                    {isStart && (
                                                        <div className="px-1 text-center animate-in fade-in zoom-in duration-300 w-full min-w-0">
                                                            <span className="block text-[10px] font-black truncate w-full uppercase tracking-tight">
                                                                {session.class_code}
                                                            </span>
                                                            <span className="block text-[8px] font-medium opacity-80 whitespace-nowrap truncate w-full">
            ...
            <div className="space-y-4 max-w-[614px] mx-auto py-2">
                {schedules.length > 0 ? (
                    schedules.sort((a, b) => a.start_time.localeCompare(b.start_time)).map((item) => (
                        <div key={item.id} className="flex flex-col sm:flex-row items-stretch gap-0 sm:gap-4 rounded-2xl border border-slate-200 bg-white hover:border-blue-400 transition-all shadow-sm overflow-hidden group">
                            <div className="flex flex-row sm:flex-col items-center justify-center sm:min-w-[140px] p-6 bg-slate-50/50 sm:border-r border-slate-100 gap-4 sm:gap-1">
                                <span className="text-2xl font-black text-blue-600">{item.start_time.slice(0, 5)}</span>
                                <div className="hidden sm:block w-8 h-[2px] bg-blue-200 my-2 rounded-full" />
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2">
                                            <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-200 text-[10px] uppercase font-bold px-2 py-0">Lớp học</Badge>
                                            <h4 className="font-bold text-xl text-slate-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{item.class_code}</h4>
                                        </div>
                                ...
                                <div className="grid grid-cols-1 gap-4 pt-2">
                                    {item.meeting_url ? (
                                        <a
                                            ...
                                            className="flex items-center gap-4 p-4 rounded-xl bg-blue-600 text-white shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all active:scale-[0.98]"
                                        >
                                        ...
                                        <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100 text-slate-400">
                    ...
                    <div className="py-24 flex flex-col items-center justify-center border-2 border-dashed rounded-3xl border-slate-200 bg-slate-50/50">
                        ...
                        <Button variant="outline" onClick={() => setViewMode("week")} className="mt-6 rounded-xl border-slate-200 text-xs font-black uppercase tracking-widest px-6 h-10 hover:bg-white hover:text-blue-600">Duyệt lịch theo tuần</Button>
                    </div>
                )}
            </div>
        )
    ...
    return (
        <div className="grid gap-8 md:grid-cols-12 min-h-[calc(100vh-140px)] animate-in fade-in duration-700">
            <div className="md:col-span-4 lg:col-span-3 space-y-6">
                ...
                <div className="p-6 rounded-[32px] bg-gradient-to-br from-blue-600 to-blue-700 text-white shadow-xl shadow-blue-200 relative overflow-hidden group">
                    ...
                    <Button variant="secondary" className="w-full h-11 rounded-2xl text-[11px] font-black uppercase tracking-widest bg-white text-blue-600 hover:bg-blue-50 border-none relative z-10 shadow-lg">
                        GỬI YÊU CẦU HỖ TRỢ
                    </Button>
                </div>
            </div>
            ...
            <div className="md:col-span-8 lg:col-span-9">
                <Card className="border-none shadow-sm bg-white min-h-full flex flex-col rounded-[32px] overflow-hidden">
                    <CardHeader className="flex flex-col sm:flex-row items-center justify-between gap-6 px-8 pt-8 pb-6 border-b border-slate-50">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-3 bg-slate-50 p-2 rounded-[20px] border border-slate-100 shadow-inner">
                            <div className="flex items-center gap-1 pr-3 border-r border-slate-200">
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    className="h-9 w-9 rounded-xl hover:bg-white hover:text-blue-600 hover:shadow-sm"
                            ...
                            <div className="flex items-center gap-1">
                                <Button
                                    ...
                                    size="sm"
                                    className={cn(
                                        "h-9 px-4 gap-2 font-black text-[10px] uppercase tracking-widest rounded-xl transition-all",
                    ...
                    <CardContent className="flex-1 p-5 sm:p-6">
                        {loading ? (
                            <div className="space-y-6">
                                <div className="grid grid-cols-[64px_repeat(7,minmax(0,1fr))] gap-0 border border-slate-100 rounded-3xl overflow-hidden">
                                    <div className="h-10 bg-slate-50/50 border-b border-slate-100" />
                                    {Array.from({ length: 7 }).map((_, i) => <div key={i} className="h-10 bg-slate-50/50 border-b border-l border-slate-100" />)}
                                    {Array.from({ length: 56 }).map((_, i) => <Skeleton key={i} className="h-10 w-full rounded-none border-b border-l border-slate-50/50 bg-slate-20/20" />)}
                                </div>
                            </div>
                        ) : (
