...
export default function ClassDetailPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await api.get(`/student/classes/${code}`);
    ...
    return (
        <div className="container mx-auto p-2 space-y-6 max-w-7xl animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 flex items-center gap-2 mt-1">
                            Mã lớp: <span className="font-mono font-medium text-indigo-600">{classData.class_code}</span>
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'Thời gian bắt đầu', value: new Date(classData.start_day).toLocaleDateString('vi-VN'), icon: Calendar, color: 'text-blue-500', bg: 'bg-blue-50' },
                    { label: 'Thời gian kết thúc', value: new Date(classData.end_day).toLocaleDateString('vi-VN'), icon: Clock, color: 'text-purple-500', bg: 'bg-purple-50' },
                    { label: 'Sĩ số học sinh', value: `${classData.student_count} học sinh`, icon: Users, color: 'text-emerald-500', bg: 'bg-emerald-50' },
                    { label: 'Đội ngũ giáo viên', value: `${classData.teacher_count} giáo viên`, icon: GraduationCap, color: 'text-amber-500', bg: 'bg-amber-50' },
                ].map((stat, idx) => (
                ...
                <div className="lg:col-span-2 space-y-6">
                        ...
                        <CardContent className="p-0">
                            <div className="divide-y divide-slate-100">
                                {classData.schedules.length > 0 ? (
                                    classData.schedules.map((session) => (
                    ...
                    <div className="space-y-4">
                        <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2 pl-1">
                            ...
                            Danh sách học sinh ({classData.student_count})
                        </h2>
                                ...
                                <Card key={student.id} className="p-0 border-none shadow-sm hover:shadow-md transition-all group overflow-hidden bg-white/80 backdrop-blur-sm shrink-0 w-fit">
                                    <CardContent className="p-3 flex items-center gap-3">
                                        <Avatar className="h-10 w-10 border border-slate-100">
                                            ...
                                            <AvatarFallback className="bg-slate-100 text-slate-500 text-xs">{student.name.charAt(0)}</AvatarFallback>
                                        </Avatar>
                                        <div className="whitespace-nowrap">
                                            <p className="text-sm font-semibold text-slate-900 group-hover:text-emerald-600 transition-colors">{student.name}</p>
                                        </div>
                                    </CardContent>
                                </Card>
                            ))}
                                ...
                                <div key={teacher.id} className="flex items-center gap-4 hover:bg-slate-50 transition-colors group">
                                    <div className="relative">
                                        <Avatar className="h-10 w-10 border-2 border-indigo-100 group-hover:border-indigo-300 transition-colors">
                                            ...
                                            <AvatarFallback className="bg-indigo-50 text-indigo-600 font-bold">{teacher.name.charAt(0)}</AvatarFallback>
                                        </Avatar>
                                    </div>
                                    <p className="font-bold text-slate-900 text-sm group-hover:text-indigo-600 transition-colors">{teacher.name}</p>
